...
>(({ className, ...props }, ref) => (
  <div className="relative w-full overflow-x-auto overscroll-x-contain [-webkit-overflow-scrolling:touch]">
    <table
      ref={ref}
      className={cn("w-full min-w-[540px] caption-bottom text-sm", className)}
      {...props}
    />
  </div>
));
...
>(({ className, ...props }, ref) => (
  <th
    ref={ref}
    className={cn(
      "h-9 px-4 text-left align-middle text-[10px] font-semibold uppercase tracking-wider text-foreground-muted",
      className
    )}
    {...props}
  />
